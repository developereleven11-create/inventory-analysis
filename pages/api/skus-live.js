...
function safeNum(v, fallback = 0){ return v === null || v === undefined ? fallback : Number(v); }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const q = `
      ...
      LEFT JOIN ( SELECT sku, SUM(quantity) as mtd_qty FROM orders_lineitems WHERE created_at >= date_trunc('month', CURRENT_DATE) AND created_at < CURRENT_DATE + INTERVAL '1 day' GROUP BY sku ) mt ON mt.sku = m.sku
      LEFT JOIN ( SELECT ol.sku, SUM(ol.quantity) as prev_qty FROM orders_lineitems ol WHERE ol.created_at >= (date_trunc('month', CURRENT_DATE) - INTERVAL '1 month') AND ol.created_at <= (date_trunc('month', CURRENT_DATE) - INTERVAL '1 month') + ((date_part('day', CURRENT_DATE)::int - 1) * INTERVAL '1 day') GROUP BY ol.sku ) prev ON prev.sku = m.sku
      WHERE m.date = $1
    ...
    console.error('skus-live error', e && e.message ? e.message : e);
    return res.status(500).json({ error: 'Failed to fetch SKUs', detail: e && e.message ? e.message : e });
  } finally {
    client.release();
  }
}
